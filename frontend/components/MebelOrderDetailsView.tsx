...
export function MebelOrderDetailsView({ order }: MebelOrderDetailsViewProps) {
  ...
  const handlePrintReceipt = async () => {
    try {
      ...
      const savedPrinter = localStorage.getItem('selected_printer') || 'Xprinter XP-80T';
      const companyName = localStorage.getItem('printer_company_name') || 'Shuhrat';
      const companyPhone = localStorage.getItem('printer_phone') || '97 677 23 01';
      const companyDescription = localStorage.getItem('printer_company_description') || 'Sifatli mebellar maskani';
      const companyAddress = localStorage.getItem('printer_company_address') || '';
      const thanksMessage = localStorage.getItem('printer_thanks_message') || 'Xaridingiz uchun rahmat!';
      ...
      order.products.forEach((it: any) => {
        ...
        const price = Number(it.priceGet) || Number(it.price) || Number(product?.price) || 0;
      ...
      const deadlineDate = (order as any).Deadline?.deadline || (order as any).deadline;
      const deadlineStr = deadlineDate ? formatDate(deadlineDate) : 'Mavjud emas';
